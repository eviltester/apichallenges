...
public class SecretTokenChallenges {
    ...
    //    GET /api/secret/note with invalid token and 401
    ...
    //    GET /api/secret/note with token and see token
    ...
    //    POST /api/secret/note with token and update secret note
    ...
    //    POST /api/secret/note with invalid token and 401
    public static ChallengeDefinitionData postMissingTokenAuth401(int challengeOrder) {
        ChallengeDefinitionData aChallenge =
                new ChallengeDefinitionData(
                        ChallengeRenderer.renderChallengeNumber(challengeOrder),
                        "POST /api/secret/note (401)",
                        "Issue a POST request on the `/api/secret/note` end point with a note payload {\"note\":\"my note\"} and receive 401 when no X-AUTH-TOKEN present");
    ...
    //    POST /api/secret/note with no token and 403
    public static ChallengeDefinitionData postInvalidTokenAuth401(int challengeOrder) {
        ChallengeDefinitionData aChallenge =
                new ChallengeDefinitionData(
                        ChallengeRenderer.renderChallengeNumber(challengeOrder),
                        "POST /api/secret/note (403)",
                        "Issue a POST request on the `/api/secret/note` end point with a note payload {\"note\":\"my note\"} and receive 403 when X-AUTH-TOKEN does not match a valid token");
    ...
    //    GET /api/secret/note with bearer token authorization
    public static ChallengeDefinitionData getWithValidBearerToken200(int challengeOrder) {
        ChallengeDefinitionData aChallenge =
                new ChallengeDefinitionData(
                        ChallengeRenderer.renderChallengeNumber(challengeOrder),
                        "GET /api/secret/note (Bearer)",
                        "Issue a GET request on the `/api/secret/note` end point receive 200 when using the X-AUTH-TOKEN value as an Authorization Bearer token - response body should contain the note");
    ...
    public static ChallengeDefinitionData postUpdateWithValidBearerToken200(int challengeOrder) {
        ChallengeDefinitionData aChallenge =
                new ChallengeDefinitionData(
                        ChallengeRenderer.renderChallengeNumber(challengeOrder),
                        "POST /api/secret/note (Bearer)",
                        "Issue a POST request on the `/api/secret/note` end point with a note payload e.g. {\"note\":\"my note\"} and receive 200 when valid X-AUTH-TOKEN value used as an Authorization Bearer token. Status code 200 received. Note is maximum length 100 chars and will be truncated when stored.");
