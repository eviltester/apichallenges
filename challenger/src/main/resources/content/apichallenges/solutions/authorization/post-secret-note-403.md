...
description: How to solve POST /api/secret/note (403) by sending an invalid X-AUTH-TOKEN value.
seo_description: Use this walkthrough to send POST /api/secret/note with a wrong X-AUTH-TOKEN, verify the 403 response, and confirm the forbidden challenge result.
...
schema_howto_steps: Create a POST request to /api/secret/note||Send a valid JSON note body||Include an X-AUTH-TOKEN header with an invalid value||Verify the response status is 403
showads: true
---
...
{{<api-live-request method="POST" path="/api/secret/note" expected-status="403" headers="X-AUTH-TOKEN: wrong-token||Accept: application/json" body='{"note":"note with wrong token"}' details="true" summary="POST /api/secret/note with a wrong auth token to trigger 403" open="true">}}
