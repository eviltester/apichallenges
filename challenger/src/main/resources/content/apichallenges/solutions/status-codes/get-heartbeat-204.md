...
description: How to solve GET /api/heartbeat (204) and verify the no-content success response.
seo_description: Use this walkthrough to send GET /api/heartbeat, verify the 204 no-content response, and confirm the request body stays empty for the challenge.
...
schema_howto_steps: Create a GET request to /api/heartbeat||Include X-CHALLENGER so progress is tracked||Send the request||Verify the response status is 204 and the body is empty
showads: true
---
...
{{<api-live-request method="GET" path="/api/heartbeat" expected-status="204" headers="Accept: */*" details="true" summary="GET /api/heartbeat to receive 204 with no body" open="true">}}
