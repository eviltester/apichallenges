...
description: How to solve PATCH /api/heartbeat (500) and trigger the simulated server error.
seo_description: Use this walkthrough to send PATCH /api/heartbeat, verify the 500 response, and see how the API exposes unsupported heartbeat behavior for the challenge.
...
schema_howto_steps: Create a PATCH request to /api/heartbeat||Include X-CHALLENGER so progress is tracked||Send the request without a body||Verify the response status is 500
showads: true
---
...
{{<api-live-request method="PATCH" path="/api/heartbeat" expected-status="500" headers="Accept: */*" details="true" summary="PATCH /api/heartbeat to trigger 500" open="true">}}
