...
- `DELETE /heartbeat` reaches a real endpoint with a disallowed method, causing `405 Method Not Allowed`.
...
- Compare `DELETE /heartbeat` with `GET /heartbeat` and inspect both raw status lines.
- Check whether the `Allow` header explains which methods `/heartbeat` accepts.