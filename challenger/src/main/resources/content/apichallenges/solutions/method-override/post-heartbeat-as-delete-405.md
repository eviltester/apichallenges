...
description: How to solve POST /api/heartbeat as DELETE using X-HTTP-Method-Override.
seo_description: Use this walkthrough to send POST /api/heartbeat with X-HTTP-Method-Override DELETE, verify the 405 response, and complete the method override challenge.
...
schema_howto_steps: Create a POST request to /api/heartbeat||Add X-HTTP-Method-Override DELETE||Include X-CHALLENGER so progress is tracked||Verify the response status is 405
showads: true
---
...
{{<api-live-request method="POST" path="/api/heartbeat" expected-status="405" headers="X-HTTP-Method-Override: DELETE||Accept: */*" details="true" summary="POST /api/heartbeat with DELETE override to trigger 405" open="true">}}
