...
description: How to solve POST /api/heartbeat as TRACE using X-HTTP-Method-Override.
seo_description: Use this walkthrough to send POST /api/heartbeat with X-HTTP-Method-Override TRACE, verify the 501 response, and complete the method override challenge.
next_challenge: /apichallenges/solutions/authentication/get-secret-token-401
...
schema_howto_steps: Create a POST request to /api/heartbeat||Add X-HTTP-Method-Override TRACE||Include X-CHALLENGER so progress is tracked||Verify the response status is 501
showads: true
---
...
{{<api-live-request method="POST" path="/api/heartbeat" expected-status="501" headers="X-HTTP-Method-Override: TRACE||Accept: */*" details="true" challenge-request="true" summary="POST /api/heartbeat with TRACE override to trigger 501">}}
