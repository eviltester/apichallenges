...
description: How to solve POST /api/heartbeat as PATCH using X-HTTP-Method-Override.
seo_description: Use this walkthrough to send POST /api/heartbeat with X-HTTP-Method-Override PATCH, verify the 500 response, and complete the method override challenge.
...
schema_howto_steps: Create a POST request to /api/heartbeat||Add X-HTTP-Method-Override PATCH||Include X-CHALLENGER so progress is tracked||Verify the response status is 500
showads: true
---
...
{{<api-live-request method="POST" path="/api/heartbeat" expected-status="500" headers="X-HTTP-Method-Override: PATCH||Accept: */*" details="true" summary="POST /api/heartbeat with PATCH override to trigger 500" open="true">}}
