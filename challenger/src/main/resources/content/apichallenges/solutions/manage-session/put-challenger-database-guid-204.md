...
- `PUT /challenger/database/{guid}` restores todo data from a previously exported database payload.
- `204 No Content` indicates the restore was accepted without returning the restored dataset.
- State restore tests need a follow-up `GET /todos` to prove the body took effect.
