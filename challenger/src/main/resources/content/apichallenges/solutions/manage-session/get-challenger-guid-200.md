...
description: How to solve GET /api/challenger/guid (200) by restoring an existing saved challenger from persistence.
seo_description: Use this walkthrough to restore a saved challenger with GET /api/challenger/{guid}, verify the 200 response, and confirm the active session changes.
...
schema_howto_steps: Create a challenger session||Allow or arrange for that saved challenger to be absent from memory||Send GET /api/challenger/{guid} with the saved challenger id||Verify the response status is 200
showads: true
---
...
{{<api-live-request method="GET" path="/api/challenger/{{currentChallenger}}" expected-status="200" headers="Accept: application/json" details="true" summary="GET /api/challenger/{guid} to get challenger progress" open="true">}}
