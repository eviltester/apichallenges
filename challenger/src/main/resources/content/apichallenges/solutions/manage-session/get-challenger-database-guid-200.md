...
description: How to solve GET /api/challenger/database/guid (200) by exporting the current todo database.
...
schema_howto_steps: Use an active challenger session||Send GET /api/challenger/database/{guid}||Save the returned todos database JSON||Verify the response status is 200
showads: true
---
...
{{<api-live-request method="GET" path="/api/challenger/database/{{currentChallenger}}" expected-status="200" headers="Accept: application/json" details="true" summary="GET /api/challenger/database/{guid} to export the todo database" open="true">}}

## Lessons Learned

- `GET /api/challenger/database/{guid}` exports todo data rather than challenge-completion metadata.
