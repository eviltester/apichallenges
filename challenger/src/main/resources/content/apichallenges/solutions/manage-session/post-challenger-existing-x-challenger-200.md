...
- `POST /challenger` with an existing `X-CHALLENGER` restores that session instead of creating a fresh one.
...
- Call `POST /challenger` with no `X-CHALLENGER`, then with an existing one, and compare status plus headers.
