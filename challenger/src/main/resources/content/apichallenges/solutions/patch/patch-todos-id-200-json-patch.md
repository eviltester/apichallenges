...
schema_howto_steps: Create a PATCH request to /api/todos/{id}||Include X-CHALLENGER so the challenge is tracked in your current session||Set Content-Type to application/json-patch+json||Send a JSON Patch operation array and verify the response status is 200
showads: true
---


# How to complete the challenge `PATCH /api/todos/id (200) json-patch`
...
{{<api-live-request method="PATCH" path="/api/todos/{{firstTodoId}}" expected-status="200" headers="Content-Type: application/json-patch+json||Accept: application/json" body='[{"op":"replace","path":"/title","value":"patched with json patch"}]' details="true" summary="PATCH /api/todos/{id} with JSON Patch to update a todo" open="true">}}
