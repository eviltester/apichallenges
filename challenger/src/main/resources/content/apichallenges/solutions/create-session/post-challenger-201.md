...
schema_howto_steps: Create a POST request to /api/challenger||Send the request with no body to create a new challenger session||Capture the returned X-CHALLENGER value for subsequent challenge requests||Send the request and verify the response status is 201
showads: true
---
...
{{<api-live-request method="POST" path="/api/challenger" expected-status="201" use-challenger="false" headers="Accept: application/json" details="true" summary="POST /api/challenger to create a new challenger session" open="true">}}
