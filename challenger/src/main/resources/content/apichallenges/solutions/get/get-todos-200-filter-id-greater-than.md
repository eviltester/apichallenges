...
schema_howto_steps: Create a GET request to /api/todos||Include X-CHALLENGER so the challenge is tracked in your current session||Add Accept application/json so the response is JSON||Add an id greater-than filter that returns some, but not all, todos||Send the request and verify the response status is 200
showads: true
---
...
{{<api-live-request method="GET" path="/api/todos?id>5" expected-status="200" headers="Accept: application/json" details="true" summary="GET /api/todos?id>5 to return todos with ids greater than 5" open="true">}}
