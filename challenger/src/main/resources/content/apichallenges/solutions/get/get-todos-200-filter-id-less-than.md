...
schema_howto_steps: Create a GET request to /api/todos||Include X-CHALLENGER so the challenge is tracked in your current session||Add Accept application/json so the response is JSON||Add an id less-than filter that returns some, but not all, todos||Send the request and verify the response status is 200
showads: true
---
...
{{<api-live-request method="GET" path="/api/todos?id<6" expected-status="200" headers="Accept: application/json" details="true" summary="GET /api/todos?id<6 to return todos with ids less than 6" open="true">}}
...
- Pair `id<` with a `GET /api/todos` baseline so you can explain every returned `id`.
