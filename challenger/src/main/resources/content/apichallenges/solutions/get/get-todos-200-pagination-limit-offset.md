...
schema_howto_steps: Create a GET request to /api/todos||Include X-CHALLENGER so the challenge is tracked in your current session||Add Accept application/json so the response is JSON||Add _limit=5 and _offset=5 to page through todos||Send the request and verify the response status is 200
showads: true
---
...
- Add `_limit=5` and `_offset=5` as URL parameters:
    - `{{<ORIGIN_URL>}}/api/todos?_limit=5&_offset=5`
...
{{<api-live-request method="GET" path="/api/todos?_limit=5&_offset=5" expected-status="200" headers="Accept: application/json" details="true" summary="GET /api/todos?_limit=5&_offset=5 to request the next page of todos" open="true">}}
