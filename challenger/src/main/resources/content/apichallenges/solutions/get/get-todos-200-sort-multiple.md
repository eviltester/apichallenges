...
schema_howto_steps: Create a GET request to /api/todos||Include X-CHALLENGER so the challenge is tracked in your current session||Add Accept application/json so the response is JSON||Add _sortBy with comma-separated todo fields||Send the request and verify the response status is 200
showads: true
---
...
- Add `_sortBy=doneStatus,-id` as a URL parameter:
    - `{{<ORIGIN_URL>}}/api/todos?_sortBy=doneStatus,-id`
...
{{<api-live-request method="GET" path="/api/todos?_sortBy=doneStatus,-id" expected-status="200" headers="Accept: application/json" details="true" summary="GET /api/todos?_sortBy=doneStatus,-id to sort todos by multiple fields" open="true">}}
