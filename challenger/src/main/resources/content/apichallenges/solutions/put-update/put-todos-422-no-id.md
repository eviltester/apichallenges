...
- `PUT /todos` without any `id` leaves the server unable to identify the resource to replace.
...
- Add only the body `id` and compare this failure with the successful `PUT /todos` body-id challenge.
- Send the same no-id payload to `POST /todos` and observe whether create semantics differ.